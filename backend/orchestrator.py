...
from nodes import call_groq_api
...
def live_agent_1_mapper(modal_text: str) -> dict:
    ...
    try:
        response = call_groq_api(system_instruction, f"User Text to Analyze:\n{modal_text}", key, "openai/gpt-oss-120b")
        if response:
            return response
...
def live_agent_2_query_creator(topic: str) -> dict:
    ...
    try:
        ...
        response = call_groq_api(sys_inst, prompt, key, "openai/gpt-oss-120b")
        if response:
            return response
    except Exception as e:
        print(f"      -> [API ERROR]: {e}")
...
def run_pre_chamber_agents(modal_answers: str, target_topic: str):
    ...
    print("\n   [AGENT 1 TRIGGERED]: COGNITIVE MAPPER (Groq openai/gpt-oss-120b)")
    ...
    print("\n   [AGENT 2 TRIGGERED]: QUERY CREATOR (Groq openai/gpt-oss-120b)")
    t0 = time.time()
